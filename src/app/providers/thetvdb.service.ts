...
export class TheTvDbService {
    ...
    ShowAndEpisodes(series_id: number): Observable<ShowEpisodeModel> {
        return Observable.combineLatest(this.tvDb.getSeries(series_id), this.tvDb.getEpisodes(series_id))
    ...
    searchSeries(name: string): Observable<SearchModel[]> {
        ...
        return this.tvDb.searchSeries({name: name}).map(search_results => {
